...
import { chat, currentTokens, extractJson, inTokenScope } from "../ai.js";
...
export function reviewMr(
  cfg: Config,
  project: string | number,
  iid: number,
  opts: { dryRun?: boolean; fullReview?: boolean; createIssues?: boolean } = {},
): Promise<ReviewResult> {
  // 独立 token 计量上下文：并发审查互不串账
  return inTokenScope(() => reviewMrImpl(cfg, project, iid, opts));
}

async function reviewMrImpl(
  cfg: Config,
  project: string | number,
  iid: number,
  opts: { dryRun?: boolean; fullReview?: boolean; createIssues?: boolean },
  ...
  if (skills.length === 0) {
    // 例如 enabled 只开了 go 而 MR 没碰 .go 文件——正常情况，不是错误
    console.error(`[review] 没有 skill 适用于本次改动的文件，跳过`);
    return {
      mr, findings: [], filtered: 0, skippedFiles: skipped, verdict: "approve",
      summary: "没有适用于本次改动的审查维度（skill 触发条件均未命中）。", incremental,
    };
  }
  ...
  recordReview({
    ...
    tokensIn: currentTokens().input, tokensOut: currentTokens().output,
    url: mr.web_url,
...
export async function testSkillOnMr(
  ...
  const prompt = redact(reviewUserPrompt(mr, files, []), cfg.review.redactPatterns);
  const out = await chat(cfg.ai, reviewSystemPrompt(cfg, skill), prompt, {
    model: skill.model,
  });
...
function buildSummary(
  ...
  if (findings.length > 0) {
    ...
    for (const f of findings) {
      const title = f.title.replace(/\|/g, "︱"); // 竖线会撑破 markdown 表格
      lines.push(`| ${SEV_LABEL[f.severity]} | \`${f.file}${f.line ? ":" + f.line : ""}\` | ${title} | ${f.skill} |`);
    }
  }
  if (skipped.length > 0) {
