...
import { recordFeedback, type FeedbackRecord } from "./store.js";
...
export async function collectFeedback(
  ...
  for (const d of botDiscussions.slice(0, 20)) {
    try {
      const awards = await gl.getNoteAwards(project, iid, d.notes[0].id);
      up += awards.filter((a) => a.name === "thumbsup").length;
      down += awards.filter((a) => a.name === "thumbsdown").length;
    } catch {
      /* 表情拉取失败不影响结算 */
    }
  }
